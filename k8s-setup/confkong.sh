...
curl -s -X POST -H "Content-Type: application/json" "http://${KONG_ADMIN_IP}:${KONG_ADMIN_PORT}/routes" -d"{\"hosts\": [\"${PRIVATE_DNS}\", \"${ELB_DNS}\"], \"paths\": [\"/count\", \"/uppercase\"], \"strip_path\":false, \"service\": {\"id\":${KONG_SVC_ID}}}" > /tmp/routeid.json
...
curl -s -XPOST -d'{"s":"some lower case string"}' -H "Host: ${ELB_DNS}" "http://${KONG_PROXY_IP}:${KONG_PROXY_PORT}/uppercase"
